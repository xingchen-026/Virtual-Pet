...
│   ├── settings_window.py    # 设置窗口（名称/性格/语气/大小/AI 配置）
...
3. 运行程序：
   ...
   - 皮肤选择窗口显示每个皮肤的缺失状态；非默认皮肤可点「补充动画」按状态补齐缺失动画
     ...
     可添加多张精灵图（同一皮肤可由不同资源拼成），每张切成逐帧，点击任意帧循环分配动画
     状态（同一状态的帧按顺序成为该动画），每张可单独镜像/抠图；②按状态分别上传图片。
     一键镜像（源图朝左→朝右）、一键抠图（自动取背景色 / 手动选色 / 点源图取色）、
     逐动画调节播放速度；左侧源图取色、右侧实时播放处理后的动画。生成后即时启用。
   - 设置窗口可修改宠物名称、分别自定义性格与语气、调节宠物大小（0.5x~2.0x）并配置
     AI 服务商 / 模型 / API Key（保存后即时生效）。宠物名称在数值面板与聊天窗口统一显示。
