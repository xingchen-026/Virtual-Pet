...
def test_custom_tone_injected_when_present():
    text = _prompt_manager(tone="高冷毒舌但内心温柔").system_prompt()
    assert "高冷毒舌但内心温柔" in text


def test_no_tone_line_when_empty():
    text = _prompt_manager(tone="").system_prompt()
    assert "你的性格与说话风格" not in text


def test_personality_tone_persisted(tmp_path):
